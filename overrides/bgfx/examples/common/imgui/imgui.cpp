...
struct OcornutImguiContext
{
	...
	void create(float _fontSize, bx::AllocatorI* _allocator)
	{
		...
		//Patched: let's just see if the default allocator works
...
//			config.MergeGlyphCenterV = true;
		...
		)
	{
		...
		ImGuiIO& io = ImGui::GetIO();
		/* Patched, don't do any input here
		if (_inputChar >= 0)
		{
...
void igBGFXCreate(float _fontSize) {
...
void igBGFXBeginFrame(uint16_t _width, uint16_t _height, uint16_t _viewId) {
	imguiBeginFrame(0, 0, 0, 0, _width, _height, -1, _viewId);
}
...
//Patched: let's just use the default allocator
//#define STBTT_malloc(_size, _userData) memAlloc(_size, _userData)
//#define STBTT_free(_ptr, _userData) memFree(_ptr, _userData)
#define STB_RECT_PACK_IMPLEMENTATION
#include <stb/stb_rect_pack.h>
// PATCHED: NanoVG should already implement STB_TRUETYPE
